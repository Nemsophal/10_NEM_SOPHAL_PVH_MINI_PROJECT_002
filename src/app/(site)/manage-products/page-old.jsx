...
export default function ManageProductsPage() {
  ...
  const handleDelete = async (productId) => {
    ...
    try {
      ...
      const response = await fetch(`/api/products/manage?id=${productId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
                  ...
                  >
                    <div className="relative aspect-square overflow-hidden rounded-lg bg-gray-100">
                      ...
                      <div className="absolute right-3 top-3 flex gap-2 rounded-lg bg-white/95 p-2 shadow-md transition opacity-0 group-hover:opacity-100 backdrop-blur-sm">
                       <button
                         onClick={() =>
                           router.push(`/manage-products/${product.id || product.productId}/edit`)
                         }
                       ...
                       <button
                         ...
                         disabled={deleting === (product.id || product.productId)}
                    ...
                    <div className="border-t border-gray-100 px-4 py-3 flex gap-2">
                     ...
                     >
                       <DeleteIcon />
                       <span>{deleting === (product.id || product.productId) ? "Deleting..." : "Delete"}</span>
                     </button>
                   </div>
                 </div>
               ))}
