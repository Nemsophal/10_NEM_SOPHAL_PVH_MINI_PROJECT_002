...
function UserAvatar() {
  ...
  const handleSignOut = async () => {
    ...
    console.log("[Navbar] Signed out, redirecting to login");
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex h-9 w-9 items-center justify-center rounded-full bg-lime-400 text-sm font-bold text-gray-900 shadow-sm transition hover:bg-lime-300"
        aria-label="User menu"
      >
        U
      </button>
      {open && (
        <div className="absolute right-0 top-11 w-36 rounded-xl border border-gray-100 bg-white py-1 shadow-lg">
          <button
      ...
      {open && (
        <div id="mobile-nav" className="border-t border-gray-100 bg-white py-3 md:hidden">
          <div className="mx-auto flex w-full max-w-7xl flex-col gap-1">
             ...
             {isLoggedIn ? (
               <button
                 onClick={async () => { setOpen(false); await signOut({ redirect: false }); router.replace("/"); }}
